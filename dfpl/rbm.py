...
class RBMLayer(tf.keras.layers.Layer):
    ...
    def call(self, X):
        ...
        visibleGen = self.__sample_prob(
            self.__prob_v_given_h(hiddenGen, self.w, self.vb)
        )
...
class RBM(tf.keras.Model):
    def __init__(self, input_size, output_size, name="RBM", **kwargs):
        ...
        self.rbm_encoder1 = RBMLayer(
            self._input_size, int(self._input_size // 2), name="encoder_0"
        )
        ...
        if self.n_layer > 1:
            for i in range(1, self.n_layer):
                ...
                self.rbm_encoder_layers.append(
                    RBMLayer(
                        int(self._input_size // factor_units) * 2,
                        int(self._input_size // factor_units),
                        name=f"encoder_{i}",
                    )
                )
            ...
            self.rbm_decoder1 = RBMLayer(
                int(input_size // factor_units) // 2,
            ...
            self.rbm_decoder_layers.append(
                RBMLayer(
                    int(input_size // 2),
                    int(input_size),
                    name=f"decoder_{self.n_layer - 1}",
                )
            )
    ...
    def train_step(self, data):
        ...
        for idx, (key, value) in enumerate(out_dict.items()):
            ...
            curr_layer.weights[0].assign(
                curr_layer.weights[0]
                + learning_rate
                * (positive_grad - negative_grad)
                / tf.cast(tf.shape(last_out)[0], dtype=tf.float32)
            )
            curr_layer.weights[1].assign(
                curr_layer.weights[1]
                + learning_rate * tf.math.reduce_mean(hiddenGen - h1, 0)
            )
            curr_layer.weights[2].assign(
                curr_layer.weights[2]
                + learning_rate * tf.math.reduce_mean(last_out - visibleGen, 0)
            )
...
def define_rbm_model(
    opts: options.Options, input_size: int = 2048, encoding_dim: int = 256
) -> Model:
    """
    ...
    if opts.aeOptimizer == "Adam":
        ...
        ac_optimizer = tf.keras.optimizers.SGD(
            learning_rate=opts.aeLearningRate, decay=opts.aeLearningRateDecay
        )
    else:
        logging.info("Wrong optimizer")
    rbm_model = RBM(input_size=input_size, output_size=encoding_dim)
    rbm_model.compile(
        optimizer=ac_optimizer, loss=my_loss
...
def train_full_rbm(df: pd.DataFrame, opts: options.Options) -> Model:
    ...
    if opts.ecWeightsFile == "":
        ...
        logging.info(
            f"RBM weights will be saved in {base_file_name}.rbm{opts.encFPSize}.hdf5"
        )
        rbm_weights_file = os.path.join(
            opts.outputDir, base_file_name + f".rbm{opts.encFPSize}.hdf5"
        )
        ec_weights_file = os.path.join(
            opts.outputDir, base_file_name + ".rbm_encoder.weights.hdf5"
        )
    else:
    ...
    callback_list = callbacks.autoencoder_callback(
        checkpoint_path=rbm_weights_file, opts=opts
    )
    ...
    logging.info(
        f"Training RBM on a matrix of shape {fp_matrix.shape} with type {fp_matrix.dtype}"
    )
    if opts.aeSplitType == "random":
        ...
        if opts.testSize > 0.0:
            if opts.aeWabTracking:
                ...
                train_data, val_data, test_data = ae_scaffold_split(
                    df, sizes=(1 - opts.testSize, 0.0, opts.testSize), balanced=True
                )
            x_train = np.array(
    ...
    ht.store_and_plot_history(
        base_file_name=os.path.join(opts.outputDir, base_file_name + ".RBM"),
        hist=auto_hist,
    )
...
def compress_fingerprints(
    dataframe: pd.DataFrame, encoder: Model, layer_num: int
) -> pd.DataFrame:
    """
    ...
    dataframe["fpcompressed"] = pd.DataFrame(
        {"fpcompressed": [s for s in hiddenGen]}, idx
    )
