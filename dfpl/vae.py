...
def define_vae_model(opts: options.Options, output_bias=None) -> (Model, Model):
    input_size = opts.fpSize
    encoding_dim = opts.encFPSize
    ac_optimizer = optimizers.Adam(learning_rate=opts.aeLearningRate, decay=opts.aeLearningRateDecay)
    ...
    if hidden_layer_count > 1:
        ...
        if opts.aeActivationFunction != "selu":
            z_mean = Dense(units=int(input_size / factor_units), activation=opts.aeActivationFunction)(encoded)
            z_log_var = Dense(units=int(input_size / factor_units), activation=opts.aeActivationFunction)(encoded)
        else:
            z_mean = Dense(units=int(input_size / factor_units),
                           activation=opts.aeActivationFunction,
                           kernel_initializer="lecun_normal")(encoded)
            z_log_var = Dense(units=int(input_size / factor_units),
                              activation=opts.aeActivationFunction,
                              kernel_initializer="lecun_normal")(encoded)
        ...
        z = Lambda(sampling, output_shape=(int(input_size / factor_units),))([z_mean, z_log_var])
        decoded = z
        # decoding layers
    ...
    autoencoder.compile(optimizer=ac_optimizer, loss=vae_loss,
                        metrics=[bce_loss, kl_loss])
...
def train_full_vae(df: pd.DataFrame, opts: options.Options) -> Model:
    ...
    if opts.aeWabTracking and not opts.wabTracking:
        wandb.init(project=f"VAE_{opts.aeSplitType}", config=opts)
    ...
    if opts.ecWeightsFile == "":
        ...
        base_file_name = os.path.splitext(basename(opts.inputFile))[0] + opts.aeType + opts.aeSplitType
        logging.info(f"(variational) encoder weights will be saved in {base_file_name}.autoencoder.hdf5")
        vae_weights_file = os.path.join(opts.outputDir, base_file_name + ".vae.weights.hdf5")
    ...
    callback_list = callbacks.autoencoder_callback(checkpoint_path=vae_weights_file, opts=opts)
        ...
        if opts.testSize > 0.0:
            ...
            if opts.aeWabTracking:
                x_train, x_test, idx_train, idx_test = train_test_split(
                    fp_matrix, np.arange(fp_matrix.shape[0]), test_size=opts.testSize, random_state=42)
            else:
                x_train, x_test, idx_train, idx_test = train_test_split(
                    fp_matrix, np.arange(fp_matrix.shape[0]), test_size=opts.testSize, random_state=42)
    ...
    # Split the data into train and test sets using scaffold split
        ...
        logging.info("Training VAE using scaffold split")
        if opts.testSize > 0.0:
            if opts.aeWabTracking:
            ...
            x_test = np.array(test_data[test_data["fp"].notnull()]["fp"].to_list(),
                              ...
                              copy=settings.numpy_copy_values)
            x_val = None
            train_indices = train_data[train_data["fp"].notnull()].index.values
            test_indices = test_data[test_data["fp"].notnull()].index.values
        else:
            x_test = None
            x_val = None
            x_train = fp_matrix
    elif opts.aeSplitType == "molecular_weight":
        logging.info("Training VAE using molecular weight split")
        if opts.testSize > 0.0:
            if opts.aeWabTracking:
