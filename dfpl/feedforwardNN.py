...
def define_nn_model_multi(
    ...
    # compile model
    model.compile(
        loss="binary_crossentropy", optimizer=my_optimizer, metrics=["accuracy"]
    )
...
def validate_multi_model_on_test_data(
    ...
    trained_model = define_nn_model_multi(
        input_size=x_test.shape[1], output_size=y_test.shape[1]
    )

    # predict values with random model
    predictions_random = pd.DataFrame(
        trained_model.predict(x_test), columns=[n for n in col_names]
    )
    ...
    f1_random = f1_score(
        y_true=y_true, y_pred=predictions_random.round(), average="weighted"
    )

    f1_trained = f1_score(y_true=y_true, y_pred=predictions.round(), average="weighted")

    predictions_random.columns = [n + "-random" for n in col_names]
    predictions.columns = [n + "-trained" for n in col_names]

    results = pd.concat([predictions_random, predictions, y_true], axis=1)
...
def train_nn_models_multi(df: pd.DataFrame, opts: options.Options) -> None:
    ...
    names_y = [
        c
        for c in df.columns
        if c not in ["cid", "id", "smiles", "fp", "inchi", "fpcompressed"]
    ]
    ...
    if opts.kFolds > 0:
        ...
        all_scores = pd.DataFrame(
            columns=[
                "fold_no",  # fold number of k-fold CV
                "loss",
                "val_loss",
                "acc",
                "val_acc",  # FNN training
                "f1_random",
                "f1_trained",
            ]
        ...
        for train, test in kfold_c_validator.split(fpMatrix, y):
            ...
            (
                ...
                model_heatmap_x,
                model_heatmap_z,
            ) = define_out_file_names(
                path_prefix=opts.outputDir,
            ...
            if opts.verbose > 0:
                logging.info(
                    "Computation time for training the multi-label FNN: "
                    + trainTime
                    + " min"
                )
            ...
            scores = validate_multi_model_on_test_data(
                x_test=fpMatrix[test],
                checkpoint_path=checkpoint_path,
                y_test=y[test],
                col_names=names_y,
                result_file=out_file_path.replace(
                    "trainingResults.txt", "predictionResults.csv"
                ),
            )
            ...
            row_df = pd.DataFrame(
                [
                    [
                        fold_no,
                        hist.history["loss"][idx],
                        hist.history["val_loss"][idx],
                        hist.history["accuracy"][idx],
                        hist.history["val_accuracy"][idx],
                        scores[0],
                        scores[1],
                    ]
                ],
        ...
        file = re.sub(
            ".hdf5",
            "scores.csv",
            re.sub("Fold-..checkpoint", "Fold-All", checkpoint_path),
        )
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        fpMatrix, y, test_size=opts.testSize
    )

    # measure the training time
    model = define_nn_multi_label_model(
        input_size=fpMatrix.shape[1], output_size=y.shape[1], opts=opts
    )
    ...
    logging.info(
        "Computation time for training the full multi-label FNN: " + trainTime + " min"
    )
    ht.store_and_plot_history(base_file_name=model_hist_path, hist=hist)
