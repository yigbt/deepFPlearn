...
class ProgramConfig(Serializable):
    ...
    output_type: str = choice("smiles", "inchikey",
                              default="smiles")  # the type of molecular representation of the output
...
def getTanimotoSimilarityFromTwoSMILES(smile1: str = "CC(C)C=CCCCCC(=O)NCc1ccc(c(c1)OC)O",
                                       smile2: str = "COC1=C(C=CC(=C1)C=O)O") -> float:
...
if __name__ == '__main__':
    ...
    if opts.input_type == 'smiles':
        ...
        df['tanimoto_similiarity'] = df.apply(lambda x:
                                              getTanimotoSimilarityFromTwoSMILES(smile1=x[opts.input_column],
                                                                                 smile2=x['smiles_harmonized']),
                                              axis=1)
        df['levenstein_distance'] = df.apply(lambda x:
                                             getLevensteinDistance(a=x[opts.input_column], b=x['smiles_harmonized']),
                                             axis=1)
    elif opts.input_type == 'inchikey':
        df['smiles_harmonized'] = df[opts.input_column].apply(getHarmonizedSmilesfrominchis)
