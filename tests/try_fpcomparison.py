...
# read both datasets
dfS = fp.importDataFile(
    "data/S_dataset_extended.pkl", import_function=fp.importSmilesCSV, fp_size=2048
)
dfS.dropna(axis=0, subset=["cid"], inplace=True)
dfS["cid"] = dfS["cid"].apply(int).astype(str)
dfD = fp.importDataFile(
    "data/dsstox_20160701.pkl", import_function=fp.importSmilesCSV, fp_size=2048
)
...
df = pd.DataFrame(
    list(zip(cid_of_interest, toxid_of_interest)), columns=["cid", "toxid"]
)
...
df["allBoolEqual"] = [
    all(s == d) for s, d in zip(df["fpSbool"].to_list(), df["fpDbool"].to_list())
]
...
df["tanimoto"] = [
    DataStructs.FingerprintSimilarity(s, d) for s, d in zip(df["fpSbin"], df["fpDbin"])
]
...
legend = [
    c + " (" + str(round(t, 2)) + ", bool: " + str(b) + ")"
    for c, t, b in zip(df["cid"], df["tanimoto"], df["allBoolEqual"])
] + [t for t in df["toxid"]]

img = Draw.MolsToGridImage(
    df["molS"].to_list() + df["molD"].to_list(),
...
df["fpcEqual"] = [
    all(s == d)
    for s, d in zip(df["fpcompressedS"].to_list(), df["fpcompressedD"].to_list())
]
