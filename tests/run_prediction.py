...
def test_predictions():
    ...
    logging.info(
        f"Predicting compounds in the input file {opts.inputFile} for association with target {opts.target}"
    )
    ...
    if opts.acFile:
        ...
        (_, encoder) = ac.define_ac_model(
            input_size=opts.fpSize, encoding_dim=opts.encFPSize
        )
    ...
    # predict
    df2 = p.predict_values(df=df, opts=opts, use_compressed=use_compressed)
